...
impl<'gc, 'context> MutationContext<'gc, 'context> {
    ...
    pub(crate) unsafe fn can_upgrade<T: 'gc + Collect>(self, ptr: NonNull<GcBox<T>>) -> bool {
        self.context.can_upgrade(ptr)
    }
}
...
impl Context {
    ...
    unsafe fn can_upgrade<T: Collect>(&self, ptr: NonNull<GcBox<T>>) -> bool {
        let gc_box = ptr.as_ref();
